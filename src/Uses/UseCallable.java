...
public class UseCallable {
	...
	public static void main(String[] args) {
		System.out
				.println("Méthode parallèle précise avec Callable pour trouver Pi :");
		piParallelCallable();
	...
	// Temps d'exécution : 190 sec = environ 3 minutes ( ce qui est environ 2x plus court que
	// minutes de mieux que le même algo en séquentiel)
	private static void piParallelCallable() {
		...
		BigDecimal step = n.divide(new BigDecimal(4), new MathContext(8,
		...
		System.out.println("Différence avec valeur exacte : "
				+ pi_exact.subtract(somme_totale).abs());
	...
	private static void piSequentiel() {
		...
		System.out.println("Temps :" + (stop - start) + "[ms]");
		System.out.println("Différence avec valeur exacte : "
				+ Math.abs(result - 3.141592653589793238462643383279));
	}
	...
	private static void piSequentielPrecis() {
		...
		for (BigDecimal i = new BigDecimal(0.5); i.compareTo(n) < 0; i = i
				.add(one)) {
			BigDecimal factor = h.multiply(i); // 2.0*(i*h) - h / 2.0
			somme_totale = somme_totale.add(h.multiply(one.divide(
					one.add(factor.multiply(factor)), mathContext)));
		}
